...
class DataAugmentForObjectDetection():
    def __init__(self, rotation_rate=.5, max_rotation_angle=5,
                 crop_rate=.5, shift_rate=.5, change_light_rate=.5,
                 add_noise_rate=.5, flip_rate=.5, max_boxes=300,
                 cutout_rate=.5, cut_out_length=50, cut_out_holes=1, cut_out_threshold=0.5):
    ...
    def _addNoise(self, img):
        ...
        # return random_noise(img, mode='gaussian', clip=True) * 1
    ...
    def dataAugment(self, img, bboxes):
        ...
        '''
        change_num = 0  # 改变的次数
        target_w, target_h = img.shape[0:2]
        # print('------')
        ...
        #     dx = (target_w - nw) // 2
        #     dy = (target_h - nh) // 2
        # #     img.resize((target_w, target_h, 3))
            new_img = np.zeros([target_w, target_h, 3])
            # new_img = Image.new('RGB', (target_w,target_h), (128, 128, 128))
            new_img = random_noise(new_img, mode='gaussian', seed=int(time.time()), clip=True) * 255
        ...
        #     # img.save('new_img.jpg')
            ...
            # Image.fromarray(np.uint8(img), mode='RGB').save('new_img.jpg')
        ...
        #
            ...
            # dx = (target_w - nw) // 2
            # dy = (target_h - nh) // 2
            # box_data = np.zeros((self.max_boxes, 4))    # 4: xmin, ymin, xmax, ymax
            # bboxes[:, [0, 2]] = bboxes[:, [0, 2]] * scale_w + dx
            # bboxes[:, [1, 3]] = bboxes[:, [1, 3]] * scale_h + dy
            # bboxes[:, 0:2][bboxes[:, 0:2] < 0] = 0
            # bboxes[:, 2][bboxes[:, 2] > target_w] = target_w
            # bboxes[:, 3][bboxes[:, 3] > target_h] = target_h
...
if __name__ == '__main__':
    # import h5py
    # h5_file = h5py.File('/home/qkh/data/gang_jin/data.h5', 'r')
    # for i in range(10):
    #     img = h5_file['image_data'][i]
    #     img = np.uint8(img)
    #     box_data =h5_file['box_data'][i]
    #     # dataAug = DataAugmentForObjectDetection()
    #     # new_img, new_box_data = dataAug.dataAugment(img, box_data)
    #     # new_img = exposure.rescale_intensity(new_img, out_range=(0, 255))
    #     new_img_IMAGE = Image.fromarray(np.uint8(img), 'RGB')
    #     draw = ImageDraw.Draw(new_img_IMAGE)
    #     for box in box_data:
    #         draw.rectangle([box[0], box[1], box[2], box[3]])
    #     new_img_IMAGE.save('augmented_imgs/' + str(i) + '.jpg')
    #     del img, box_data, new_img_IMAGE, draw

    from pascal_voc_parser import get_pascal_detection_data
    ...
    origin_img, origin_box = get_random_data(all_imgs[0], (960, 960), random=False)
    origin_img = exposure.rescale_intensity(origin_img, out_range=(0, 255))
    origin_img = np.uint8(origin_img)

    # Image.fromarray(origin_img, 'RGB').save('augmented_imgs/origin_image.jpg')

    dataAug = DataAugmentForObjectDetection()
    for i in range(64):
        print(i)
        new_img, new_box_data = dataAug.dataAugment(origin_img, origin_box)
        new_img = np.uint8(new_img)
        # new_img = exposure.rescale_intensity(new_img, out_range=(0, 255))
        new_img_IMAGE = Image.fromarray(new_img, 'RGB')
        draw = ImageDraw.Draw(new_img_IMAGE)
        for box in new_box_data:
            draw.rectangle([box[0], box[1], box[2], box[3]])
        new_img_IMAGE.save('augmented_imgs/' + str(i) + '.jpg')
